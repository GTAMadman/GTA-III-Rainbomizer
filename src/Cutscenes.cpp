...
void Cutscenes::RandomizeSpecialModelToLoad (int modelId, const char *modelName,
					     int flags)
{
	const char* model = GetRandomModel (modelName);
	CStreaming::RequestSpecialModel (modelId,
					 model,
					 flags);
...
{
	FILE *modelsFile = fopen ("CutsceneModels.txt", "r");
	mModels.clear ();
	...
	if (InitialiseModelData ())
		{
			...
			plugin::patch::RedirectCall (
				0x40ADB2, RandomizeSpecialModelToLoad);
			plugin::patch::RedirectCall (
				0x4474C4, RandomizeSpecialModelToLoad);
		}
}
